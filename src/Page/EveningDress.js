...
const EveningDress = () => {
  ...
  const EveningDress = products.filter((product) => product.categories === "EveningDress");
        ...
        {EveningDress.map((product) => {
          ...
          return (
            <div key={product.id} className="border p-4 rounded-lg flex flex-col justify-between">
              <div>
                <img src={product.images.url} alt='products' className="mb-2" />
              ...
              <div className="flex justify-between items-center mt-4">
                <Link className='text-xl text-gray-600' to={`/DetailsPage/${product.id}`}>
                  <RiListCheck2 />
                </Link>
                <div>
                  {quantity === 1 && (
                    <button className='border-red-700 p-1 rounded-md' onClick={() => addToCart("REMOVE_ITEM", product)}>
                      <MdDeleteOutline />
                    </button>
                  )}
                  {quantity > 1 && (
                    <button className='border-red-700 p-1 rounded-md' onClick={() => addToCart("DECREMENT", product)}>-</button>
                  )}
                  {!!quantity && <span className="mx-2">{quantity}</span>}
                  {quantity === 0 ? (
                    <button className='text-xl text-gray-600' onClick={() => addToCart("ADD_TO_CART", product)}>
                      <TbShoppingBagPlus />
                    </button>
                  ) : (
                    <button className='text-xl text-gray-600' onClick={() => addToCart("INCREMENT", product)}>+</button>
                  )}
                </div>
              </div>
            </div>
